...
__kernel void
max_project_float(__global float *d_output, // probably Nx x Ny
                  __global float *d_alpha_output, // same
                  __global float *d_depth_output, // same
                  uint Nx, uint Ny, // width & height of output (also compute grid?)
                  float boxMin_x, // view clip box ?
                  float boxMax_x,
                  float boxMin_y,
                  float boxMax_y,
                  float boxMin_z,
                  float boxMax_z,
                  float minVal, // intensity clip
                  float maxVal,
                  float gamma,
                  float alpha_pow,
                  int numParts, // scale the number of steps
                  int currentPart, // ????
                  __QUALIFIER_CONSTANT float* invP, // matrix ? 
